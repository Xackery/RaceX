...
[RequireComponent (typeof (AxisTankController))]
public class Drivetrain : BaseTankMod {

	public Clutch clutch;
	public AxisTankController tankController;
	...
	void Awake(){
		...
		clutch = new Clutch();
		tankController = GetComponent <AxisTankController>();
...
//		float m_drivetrainInertia=drivetrainInertia*powerMultiplier*externalMultiplier;
		...
		if ((rpm >= engageRPM || engaging==true) && autoClutch==true && tankController.clutchInput==0 && clutch.GetClutchPosition()!=1 && tankController.handbrakeInput==0 && ratio!=0)  {
			EngageClutch();
		}
	...
	void DoGearShifting(){
		...
		else
		{
			if (throttle<=idlethrottle+idlethrottle/10 || automatic==false) {
				...
				// engage clutch
				if (shiftTimer >=1) {
					if (rpm < engageRPM || tankController.clutchInput!=0) changingGear=false;
				}
			}
		}
	}
